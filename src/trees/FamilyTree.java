...
public class FamilyTree
{
    ...
    private static class TreeNode
    {
        ...
        ArrayList<TreeNode> collectAncestorsToList()
        {
            ...
            while(node.parent.parent != null) {
            	ancestors.add(node.parent.parent);
            	node = node.parent; 
            }
            return ancestors;
        }
	...
	private void addLine(String line) throws TreeException
	{
		...
		else
		{
			...
			if (parentNode == null)
				throw new TreeException("Parent node not foound in tree");//If the parent node wasn't found, there must have been something wrong in the 
				//data file. Throw an exception.
		}
		
		// Add child nodes to parentNode.
		for(String child: childrenArray) {
			parentNode.addChild(new TreeNode(child));//For each name in childrenArray, create a new node and add that node to parentNode.
		}
	}
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("name1 is not found"); //Throw a TreeException with a useful message
		TreeNode node2 = root.getNodeWithName(name1);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("name1 is not found"); //Throw TreeException with a useful message
